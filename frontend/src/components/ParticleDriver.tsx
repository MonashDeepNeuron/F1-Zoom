...
const ParticleDriver = () => {
  ...
  useEffect(() => {
    ...
    if (!ctx) return; // FIX: Add null check
    ...
    let animationId: number; // Add explicit type

    const img = new Image();
    img.crossOrigin = "Anonymous";
    img.src = "/driver-outline.png";

    img.onload = () => {
      const offscreen = document.createElement("canvas");
      const offCtx = offscreen.getContext("2d");
      if (!offCtx) return; // Add null check
      ...
      const lines: Line[] = []; // Add explicit type
    ...
    const draw = () => {
      ...
      if (!imageLoadedRef.current || startTimeRef.current === null) return; // FIX: Add null check
